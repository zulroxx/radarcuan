...
from rate_limiter import sync_rate_delay, async_rate_delay

ROOT_DIR = Path(__file__).parent

logging.getLogger("yfinance").setLevel(logging.ERROR)

TZ_CACHE_DIR = ROOT_DIR / "agent_cache" / "yfinance_tz"
...
SECTOR_MACRO_SENSITIVITY = {
    "Keuangan": ["bi_rate", "fed_rate", "gdp", "bond_yield", "us_10y_yield", "sp500"],
...
def fetch_live_data() -> Dict[str, Optional[float]]:
    ...
    errors = []
    for i, (key, ticker) in enumerate(YFINANCE_TICKERS.items()):
        if i > 0:
            sync_rate_delay(2.0)
        # Retry up to 4x with exponential backoff on rate limit
        for attempt in range(4):
            try:
                ...
                if is_rate_limit and attempt < 3:
                    wait = 2.0 * (4 ** attempt)
                    logger.warning(f"Rate limited on {key}, retry {attempt+1}/4 in {wait:.0f}s")
                    time.sleep(wait)
                else:
                    ...
                    break
    if errors:
        update_agent_status("macro", "error", "; ".join(errors))
    else:
        update_agent_status("macro", "ok")
    return results


YFINANCE_TIMEOUT = 60  # timeout per yfinance call in seconds

async def _fetch_with_timeout(coro, timeout=YFINANCE_TIMEOUT):
    """Run coro with timeout, returning None on timeout/failure."""
    try:
...
async def fetch_live_data_async() -> Dict[str, Optional[float]]:
    """Parallel async version of fetch_live_data with rate limiting and retries."""

    _sem = asyncio.Semaphore(1)

    async def fetch_one(key: str, ticker: str, retries: int = 5) -> Tuple[str, Optional[float]]:
        for attempt in range(retries):
            try:
                async with _sem:
                    await async_rate_delay(2.0)
                    tk = await _fetch_with_timeout(asyncio.to_thread(yf.Ticker, ticker))
                    if tk is None:
                        raise asyncio.TimeoutError("yfinance Ticker timeout")
                    hist = await _fetch_with_timeout(
                        asyncio.to_thread(lambda t=tk: t.history(period="1d"))
                    )

                    if hist is None:
                        raise asyncio.TimeoutError("yfinance history timeout")
                    if not hist.empty:
                        price = hist["Close"].iloc[-1]
                        if key == "usd_idr":
                        ...
                        info = await _fetch_with_timeout(
                            asyncio.to_thread(lambda t=tk: t.info)
                        )
                        if info and "regularMarketPrice" in info:
                            price = info["regularMarketPrice"]
                            if price:
                ...
                msg = str(e).lower()
                is_rate_limit = "too many requests" in msg or "429" in msg
                is_timeout = isinstance(e, asyncio.TimeoutError) or "timeout" in msg
                if (is_rate_limit or is_timeout) and attempt < retries - 1:
                    wait = 4.0 * (2 ** attempt) if is_timeout else 4.0 * (4 ** attempt)
                    logger.warning(
                        f"{'Timeout' if is_timeout else 'Rate limited'} on {key}, "
...
def get_macro_indicators(refresh: bool = False) -> Dict[str, Any]:
    ...
    # Cache miss — return defaults instead of hitting yfinance
    # (scheduler handles live fetching via fetch_live_data_async)
    logger.info("Macro cache miss — returning default values")
    timestamp = now_iso()
    indicators = []
...
def get_macro_summary() -> str:
    ...
    for ind in data["indicators"]:
        ...
        lines.append(f"- {ind['label']}: {ind.get('value', ind.get('defaultValue', 'N/A'))} ({trend}) — {impact}")
    return "\n".join(lines)
