...
function getYieldFill(yieldValue) {
  return yieldValue >= 5 ? "hsl(var(--cuan))" : "hsl(var(--muted-foreground))";
}
    ...
    return (
      <div className="space-y-4" data-testid="company-detail-tab-content-dividend">
        <Card className="border-border bg-card">
          <CardHeader>
            <CardTitle className="text-lg text-foreground">Riwayat dividen</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">Data riwayat dividen belum tersedia untuk saham ini.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-4" data-testid="company-detail-tab-content-dividend">
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-[1fr_1fr]">
        <Card className="border-border bg-card" data-testid="company-detail-dividend-history-card">
          <CardHeader>
            <CardTitle className="text-lg text-foreground">Riwayat dividen</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-border bg-card" data-testid="company-detail-dividend-chart-card">
          <CardHeader>
            <CardTitle className="text-lg text-foreground">Payout & yield trend</CardTitle>
          </CardHeader>
          <CardContent>
            <DeferredChart active={active} className="h-[300px] w-full" testId="company-detail-dividend-chart">
              {({ height, width }) => (
                <BarChart data={company.dividendHistory} height={height} width={width}>
                  ...
                  <Legend />
                  <Bar dataKey="payoutRatio" fill="hsl(var(--foreground))" name="Payout Ratio" radius={BAR_RADIUS} />
