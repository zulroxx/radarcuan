...
function MetricCard({ item }) {
  ...
  return (
    <Card className="border-border bg-card" data-testid={`company-detail-metric-${item.label.toLowerCase().replace(/\s+/g, "-")}`}>
      <CardContent className="flex items-center justify-between gap-3 p-3 sm:p-4">
        <div className="min-w-0">
          ...
          <Badge className={`mt-1.5 rounded-md border px-2 py-0.5 text-[10px] font-medium ${toneClasses[item.tone]}`} data-testid={`company-detail-metric-tone-${item.label.toLowerCase().replace(/\s+/g, "-")}`} variant="outline">
            {badgeLabel}
          </Badge>
        </div>
        <item.icon className="h-6 w-6 shrink-0 text-muted-foreground sm:h-7 sm:w-7" />
      </CardContent>
    </Card>
  );
}
...
function HighlightCard({ item }) {
  return (
    <Card className="border-border bg-card" data-testid={`company-detail-highlight-${item.label.toLowerCase().replace(/\s+/g, "-")}`}>
      <CardContent className="flex items-center justify-between gap-3 p-3 sm:p-4">
        <div className="min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{item.label}</p>
          <p className="mt-1 text-sm font-semibold text-foreground sm:text-base">{item.value}</p>
        </div>
        <item.icon className="h-6 w-6 shrink-0 text-muted-foreground sm:h-7 sm:w-7" />
      </CardContent>
    </Card>
