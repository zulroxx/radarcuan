...
function BenchmarkCard({ item }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4" data-testid={`company-detail-benchmark-${item.label.toLowerCase().replace(/\s+/g, "-")}`}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
        <div>
          <p className="text-slate-500">Emiten</p>
          <p className="font-semibold text-slate-950">{item.formatter(item.companyValue)}</p>
        </div>
        <div>
          <p className="text-slate-500">Rata-rata sektor</p>
          <p className="font-semibold text-slate-950">{item.formatter(item.sectorValue)}</p>
        </div>
      </div>
    </div>
  );
...
function ListCard({ icon: Icon, items, testIdPrefix, title, toneClass }) {
  return (
    <Card className="border-slate-200 bg-white" data-testid={`${testIdPrefix}-card`}>
      <CardHeader>
      ...
      <CardContent>
        <ul className="space-y-3">
          {items.map((item, index) => (
            <li key={item} className="flex items-start gap-3 text-sm text-slate-700" data-testid={`${testIdPrefix}-${index + 1}`}>
  ...
  return (
    <div className="space-y-4" data-testid="company-detail-tab-content-overview">
      <div className="grid gap-4 lg:grid-cols-1 xl:grid-cols-[1.2fr_0.8fr]">
        <Card className="border-slate-200 bg-white" data-testid="company-detail-financials-card">
          <CardHeader>
            <CardTitle className="text-lg text-slate-950">Mini Financials</CardTitle>
            <p className="text-sm text-slate-500" data-testid="company-detail-financials-description">Visualisasi revenue versus net income dalam 4 kuartal terakhir.</p>
          </CardHeader>
          <CardContent>
            {hasQuarterlyData ? (
              ...
              <p className="text-sm text-slate-500">Data kuartalan belum tersedia untuk saham ini.</p>
            )}
          </CardContent>
        </Card>

        <Card className="border-slate-200 bg-white" data-testid="company-detail-benchmark-card">
          <CardHeader>
            <CardTitle className="text-lg text-slate-950">Benchmark vs sektor</CardTitle>
            <p className="text-sm text-slate-500" data-testid="company-detail-benchmark-description">Perbandingan sederhana dengan rata-rata {company.industry} ({sectorBenchmark.peerCount} emiten).</p>
          </CardHeader>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {hasStrengths ? (
          ...
          <Card className="border-slate-200 bg-white">
            <CardHeader>
              <CardTitle className="text-lg text-slate-950">Key highlights</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-slate-500">Data kekuatan belum tersedia.</p>
            </CardContent>
          </Card>
        )}
        {hasRisks ? (
          ...
          <Card className="border-slate-200 bg-white">
            <CardHeader>
              <CardTitle className="text-lg text-slate-950">Risiko utama</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-slate-500">Data risiko belum tersedia.</p>
            </CardContent>
          </Card>
        )}
      </div>
