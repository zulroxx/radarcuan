...
- **Order Book Simulation** — AI-generated portfolio simulation across 1M/3M/6M/12M timeframes with append-only protection
...
│   ├── db_guard.py            # Append-only MongoDB wrapper (order book protection)
│   ├── order_book_store.py    # Append-only order book storage manager
...
| GET | `/api/order-book/simulation` | Latest order book simulation (support `?refresh=true`) |
| GET | `/api/order-book/history` | Paginated order book history |
| GET | `/api/order-book/history/{snapshot_id}` | Single order book snapshot by ID |
| GET | `/api/order-book/accuracy` | Accuracy metrics across all closed positions |
...
| `/order-book` | Order Book — portfolio simulation with append-only protection |
...
### Order Book Simulation Agent
Generates portfolio simulation snapshots across 4 timeframes (1M/3M/6M/12M):
- Uses sector predictions + stock recommendations as input
- Fetches live prices via Yahoo Finance for new tickers
- **Append-only**: once a stock is recorded, it is never modified or deleted
- Tracks open (active) and closed (expired) positions with accuracy metrics

## Append-Only Guard (Order Book Protection)

The order book uses a multi-layer **append-only protection system** to prevent AI agents or code from modifying or deleting existing data:

| Layer | Mechanism |
|-------|-----------|
| **MongoDB Driver** | `AppendOnlyCollection` wrapper (`db_guard.py`) blocks all `update_*`, `delete_*`, `replace_*`, `drop` operations — raises `PermissionError` |
| **Storage Manager** | `OrderBookStore` (`order_book_store.py`) exposes only `append_snapshot()` as the single write method. All read operations use dedicated methods (`get_latest_snapshot`, `get_history`, etc.) |
| **File Fallback** | Write validation ensures no existing entries are lost after file save |
| **API Layer** | All order book endpoints are read-only (`GET`). No `POST/PUT/DELETE` endpoints exist for order data |

This guarantees that data integrity is preserved even if an AI agent attempts destructive operations.
